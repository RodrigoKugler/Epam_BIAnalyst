...
- Update `shared/glossary.md` with:
  - **Pre-attentive Attributes**: Visual properties processed rapidly by the human brain.
  - **Row-Level Security (RLS)**: Mechanism restricting data visibility by user role.
  - **Semantic Model**: Layer abstracting raw data into business-friendly entities/measures.
  - **LOD Expression**: Tableau calculation controlling aggregation granularity.
  - **DAX**: Data Analysis Expressions language for Power BI.
...
Use this module as the go-to reference for visualization excellence and tool fluency within the BI Analyst training journey.
